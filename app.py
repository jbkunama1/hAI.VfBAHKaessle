    ...
    def create_app(test_config=None):
        ...
        def is_admin_user(user):
            if user is None:
                return False
            # Entweder Flag aus DB oder über ENV-Liste der Admin-Namen
            if user["is_admin"]:
                return True
        ...
        def register():
            if request.method == "POST":
                ...
                # Ersten Benutzer automatisch als Admin markieren (optional)
        ...
        def admin_export_month():
            ...
            output = []
            output.append(["username", "date", "amount"])
            for r in rows:
                output.append([r["username"], r["drinking_date"], r["amount"]])

            si_lines = []
            for row in output:
                si_lines.append(",".join(str(col) for col in row))
            csv_data = "
".join(si_lines)
